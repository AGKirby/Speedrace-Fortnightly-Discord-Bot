...
@bot.command(
    name='init',
    help=
    "*Initalizes a competition of Speedrace Weekly. No spaces in category names. (Moderators only)"
)
...
@bot.command(
    name='set-winners',
    help="*Set the winners of the phase 1 or phase 3 race (Moderators only)")
@commands.has_role("Moderators")
...
@bot.command(
    name='edit',
    help="Edit your own submitted run, use: -edit [runId] [field] [value]")
async def editRuns(ctx, runId=None, field=None, newValue=None, newValue2=None):
    try:
        if (newValue2 != None and isinstance(newValue, str)
                and isinstance(newValue2, str)):
            newValue += " " + newValue2
        if (speedrace.game == None):
            await ctx.send("Error: No game currently in progress.")
        elif (runId == None or field == None or newValue == None):
            await ctx.send(
                "Error: Invalid arguments, use: -edit [runId] [field] [value]")
        else:
            user = ctx.author
            role = discord.utils.find(lambda r: r.name == 'Moderators',
                                      ctx.message.guild.roles)
            # print(user.roles)
            if role in user.roles:
                user = "Moderator"
            await ctx.send(speedrace.editRun(str(user), runId, field,
                                             newValue))
    except:
        await ctx.send("Error")
...
@bot.command(name='points',
             help="Returns the points for each phase (and speedrun category)")
...
@bot.command(
    name='standings',
    help="Returns the current point standings of competitors with points")
...
@bot.command(name='submit',
             help="Submit a speedrun, use: -submit [category] [time] [link]")
async def submit(ctx, cat=None, time=None, link=None, name1="", name2=""):
    try:
        # print(name1 + ", " + name2)
        if (name2 != ""):
            name1 += " " + name2
        ...
        if (speedrace.game == None):
            ...
            await ctx.send(
                "Error: Invalid arguments, use: -submit [category] [time] [link]"
            )
        else:
            if (name1 == ""):
                name1 = ctx.author
            role = discord.utils.get(ctx.message.guild.roles,
                                     name='Competitors')
            message = speedrace.submitRun(
                str(name1) + ";" + cat + ";" + time + ";" + link)
...
@bot.command(name='category-runs',
             help="Shows personal bests, use: -run [category-optional]")
async def categories(ctx, category="all"):
    try:
        if (speedrace.game == None):
            await ctx.send("Error: No game currently in progress.")
        else:
...
@bot.command(name='get-run',
             help="Shows a run (or all runs), use: -run [runId-optional]")
async def getRuns(ctx, runId=0):
    try:
        if (speedrace.game == None):
            await ctx.send("Error: No game currently in progress.")
        else:
...
@bot.command(name='set-day',
             help="*Make Daily Phase 2 Announcement (Moderators only)")
@commands.has_role("Moderators")
async def sendCurrentDay(ctx, day):
    try:
...
async def dailyMessage():
    ...
    await channel.send(
        ("{} " + speedrace.p2DailyAnnouncement()).format(role.mention))
...
@bot.command(name='daily-message',
             help="*Make Daily Phase 2 Announcement (Moderators only)")
@commands.has_role("Moderators")
async def sendDailyMessage(ctx):
    try:
        ...
        if (canSend == ""):
            ...
            await ctx.send("Speedrace Bot refused to send a daily message: " +
                           canSend)
    except:
        await ctx.send("Error")
...
@bot.command(
    name='force-daily-message',
    help=
    "*Force the Bot to make the Daily Phase 2 Announcement (Moderators only)")
@commands.has_role("Moderators")
...
@bot.command(
    name='set-dm-channel',
    help=
    "*Sets the channel for the bot to make daily announcements (Moderators only)"
)
@commands.has_role("Moderators")
async def setDMChannel(ctx, id=None):
    try:
        if (id == None):
            await dm_setup(ctx)
        else:
